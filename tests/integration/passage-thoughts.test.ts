...
import { DELETE as DELETE_REPLY } from "@/app/api/thought-replies/[replyId]/route";
...
function request(path: string, method: string, cookie = author.cookie, body?: object) {
  // 写路由统一先读 JSON 请求体，GET/HEAD 不能带体
...
it("creates a public thought and personal mark atomically, preserving quote and revision", async () => {
  ...
  const { id } = (await result.json()).thoughts[0]; const listed = await read();
...
it("visibility change removes the whole conversation publicly and restores it on publication", async () => {
  const { id } = (await (await create()).json()).thoughts[0]; expect((await reply(id)).status).toBe(201);
...
it("agrees are unique/idempotent, cannot target self or private thoughts, and order thoughts", async () => {
  const first = (await (await create()).json()).thoughts[0]; await create("public", { content: "较新的感想" });
...
it("deletion keeps replies as a redacted placeholder, removes empty thoughts and never removes marks", async () => {
  const { id } = (await (await create()).json()).thoughts[0]; await reply(id);
  ...
  const empty = (await (await create()).json()).thoughts[0]; await DELETE(request("/", "DELETE"), context(empty.id));
...
it("moderation cannot reach another author's private reply thread; reply author can still delete their own record", async () => {
  const { id } = (await (await create()).json()).thoughts[0]; const response = await reply(id); const replyRow = await response.json();
...
it("lock blocks replies including admins but allows creating thoughts and marks", async () => {
  ...
  const result = await create(); expect(result.status).toBe(201); const { id } = (await result.json()).thoughts[0];
...
it("page deletion hides the whole thread while authors can manage their own retained records", async () => {
  const { id } = (await (await create()).json()).thoughts[0]; await getDb().update(pages).set({ deletedAt: new Date() }).where(eq(pages.id, termId));
