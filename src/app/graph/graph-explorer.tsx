...
// 全站图谱的交互层（T11 / #97 研究终端）：搜索定位节点（定位 = 视口居中 + 放大 + 高亮聚焦邻居）。
// 搜索是纯客户端过滤（节点数据已在手，无需再打端点）；回车定位首个命中。
// 工具栏（搜索 + 概要计数 + 视口控件）与图例由 GraphCanvas 的终端外框承载。
        ...
        <>
          <div className="relative w-full sm:w-72">
            <input
              ...
              className="w-full min-h-11 rounded-md border border-input bg-background px-3 py-1.5 text-sm outline-none focus:border-ring"
            />
            {suggestionsOpen && matches.length > 0 && (
              <ul
                className="absolute left-0 right-0 top-full z-50 mt-1 overflow-hidden rounded-md border border-border bg-popover text-popover-foreground shadow-lg"
                role="listbox"
                aria-label="匹配词条"
              >
                {matches.map((node) => (
          ...
          <span className="font-mono text-xs text-muted-foreground" data-testid="graph-summary">
            {data.nodes.length} 个词条 · {data.edges.length} 条双链关系
          </span>
          {located && (
            <span className="min-h-11 text-sm leading-snug" data-testid="graph-located" role="status">
              已定位：
              <Link
