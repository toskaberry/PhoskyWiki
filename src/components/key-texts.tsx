      ...
      {value.map((item, index) => <div key={index} className="flex flex-col gap-3 rounded-md border border-border bg-muted/30 p-3">
        <div className="grid gap-3 sm:grid-cols-2">
          {fields.map(([field, label]) =>
            <label key={field} className="flex flex-col gap-1 text-sm">{label}<input aria-label={`${label} ${index + 1}`} value={item[field] ?? ""} className="h-9 rounded-md border border-input bg-background px-3 text-sm font-normal outline-none transition-colors focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50" onChange={e => onChange(value.map((row, i) => i === index ? { ...row, [field]: e.target.value } : row))} /></label>)}
        </div>
